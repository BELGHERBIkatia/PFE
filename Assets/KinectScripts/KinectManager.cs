...
public class KinectManager : NetworkBehaviour
{
    ...
    // ****************Déclarations Partie faceTracking****************
    ...
    [SyncVar] public String KinectState = "Not Initialized";
    [SyncVar] public bool isTrackingFace = false;
	...
	{/*
        ...
        */
        if (UserId == Player1ID)
        {
            ...
            //player1Pos.y = player1Pos.y + 10;
            return player1Pos;
        }
           
        else if (UserId == Player2ID)
        {
            //player2Pos.y = player2Pos.y + 10;
            return player2Pos;
        }
...
//				break;
	...
	void Start()
	{
		...
		catch (Exception e)
		{
            ...
            CmdSendMyStateToServer("ErreurInit");
            if (CalibrationText != null)
				CalibrationText.GetComponent<GUIText>().text = message;
	...
	void ProcessSkeleton()
	{
		...
		for(int i = 0; i < KinectWrapper.Constants.NuiSkeletonCount; i++)
		{
			...
			if(skeletonData.eTrackingState == KinectWrapper.NuiSkeletonTrackingState.SkeletonTracked)
			{
                ...
                //Update Serveur Frame and Skeleton Data
               
                string skeletonFrameString = JsonUtility.ToJson(skeletonFrame);
                CmdUpdateFrame(skeletonFrameString);

                string skeletonDataString = JsonUtility.ToJson(skeletonFrame.SkeletonData[i]);
                CmdUpdateSkeletonData(skeletonDataString);
...
//				posJoint.x = KinectWrapper.Constants.ImageWidth - posJoint.x - 1;
    ...
    [Command]
    void CmdSendMyStateToServer(String state)
    {
        KinectState = state;
    }

    [Command]
    void CmdSendFaceTrackingToServer(bool FaceTrack)
    {
        isTrackingFace = FaceTrack;        
    }
    ...
    void StartFacetracker()
    {
        ...
        catch (DllNotFoundException ex)
        {
            ...
            UnityEngine.Debug.Log("Please check the Kinect and FT-Library installations.");
        }
        catch (Exception ex)
        {
            UnityEngine.Debug.LogError(ex.ToString());
    ...
    //FaceTrackingProcess fonction
    public void FacetrackingProcess()
    {
        ...
        if (facetrackingInitialized)
        {
            ...
            Debug.Log("isTrackingFace = "+isTrackingFace);
            CmdSendFaceTrackingToServer(isTrackingFace);
    ...
    [SyncVar]
    public KinectWrapper.NuiSkeletonFrame myFrame;
    [SyncVar]
    public bool isFrameInitialised = false;

    public string frameString = "";
    public string skeletonDataString = "";

    [Command]
    public void CmdUpdateFrame(string frame)
    {
    ...
    [Command]
    public void CmdUpdateSkeletonData(string data)
    {
        if (data != null)
        {
            isFrameInitialised = true;
            skeletonDataString = data;
        }
