...
public class initCalibration : MonoBehaviour
{
	...
	Stack<int> clientCalibraded = new Stack<int>();//Chaque fois qu'un utilisateur est calibré, son identifiant est empilé sur la pile.


    int nbKinectMax = 0;
    bool matrixIsCalculated = false;// vérifier si la matrice de transformation est calculé ou non 
    /*
    ...
    {
		// sauvgarder le calibrage effectuer dans le fichier calibrage.json pour l'utiliser aprer si on ne veut pas refaire le calibrage 
        A1 a1 = new A1();
        a1.a1 = new List<A2>();
    ...
    public void getCalibrageMatrixes()// cette fonction est appelé dans l'interface calibrage si on clique sur le bouton oui 
    {
		// lire le fichier calibrage.json pour récupéré le dernier calibrage sauvgardé
    ...
    void Start()
    {
		...
		collector = GetComponent<DataCollecter>();//les données clients

		MySkeletonFrameTable = collector.GetMySkeletonFrameTable(); //tous les squeletes de tous les clients
        ...
        lock (locker)
        {
            matriceCalculed = new bool[nbKinectMax][];// matrice des booleans de taille de nombre de kinect initialiser a false
