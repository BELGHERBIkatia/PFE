...
//namespace UI.Dates
//{
public class InterfaceAdmin : MonoBehaviour
{
	public GameObject accueil;
	public GameObject interfaceOperateur;
	public GameObject interfaceAdmin;
	...
	public Text sexetext;
	public Text sexetextP;
	...
	public List<string> sexeList = new List<string> (){"Homme", "Femme"};
	public string sexe;
	...
	public void deconnexion()
	{
		...
		interfaceOperateur.SetActive(false);
		interfaceAdmin.SetActive(false);
	...
	public void crudMedecin(GameObject g)
	{
		...
		case "Valider":
			...
			//string sexe= dropdownItemSelected(dropdown);
			...
			if ((IsAllDigits (nvMdp.text) == true)||(emailIsValid(nvEmail.text)==false)||(nvNumTel.text.Length!=9)){
	...
	//verifier le mdp si contient que des chifre
	...
	//vérifier la forme de l'émail( à changer)
	...
	/*public string dropdownItemSelected(Dropdown drop){
	...
	*/
	...
	public void dropdownStartP(){
		dropdownP.AddOptions (sexeList);
	}
	...
	public bool emailIsValid(String emailString){

	bool isEmail = Regex.IsMatch(emailString,@"\^[a-zA-Z0-9.!#$%&’*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*\Z", RegexOptions.IgnoreCase);
		return 	isEmail;

	}

	/*static void Main(string[] args){
		InterfaceAdmin interfacAd = new InterfaceAdmin ();
		bool ver = interfacAd.emailIsValid ("ik_belgherbi@esi.dz");
		Console.WriteLine ("votre mail est : ", ver);
	}*/
